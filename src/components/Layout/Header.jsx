...
const Header = () => {
  ...
  const quantidadeCarrinho = carrinhoState.itens.reduce(
        ...
        <ul className="navbar-nav d-flex float-end flex-row justify-content-between justify-content-md-around justify-content-lg-evenly w-75">
          ...
          {quantidadeCarrinho > 0 && (
            <li>
              <Link
                to="/carrinho"
                className="nav-item d-flex flex-column justify-content-center align-items-center text-light text-decoration-none position-relative"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="1em"
                ...
                <span>Carrinho</span>
                {quantidadeCarrinho > 0 && (
                  <span className="badge bg-danger ms-2 position-absolute top-0 end-0">
                    {quantidadeCarrinho <= 9 ? quantidadeCarrinho : "9+"}
                  </span>
                )}
              </Link>
            </li>
          ...
          {temPedido > 0 && (
            <li className="nav-item d-flex flex-column justify-content-center align-items-center">
              ...
              >
                ...
                <span>Pedidos</span>
                {quantidadePedidosNaoFinalizados > 0 && (
                  <span className="badge bg-danger ms-2 position-absolute top-0 end-0">
                    {quantidadePedidosNaoFinalizados <= 9
                      ? quantidadePedidosNaoFinalizados
                      : "9+"}
                  </span>
                )}
              </Link>
